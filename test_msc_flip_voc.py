...
parser.add_argument("--model_path",
                    default=r"E:\ryl\WeCLIP+3-coco\scripts\work_dir_voc\checkpoints\fcm+scsa\wetr_iter_30000.pth",
                    type=str, help="model_path")


def validate(model, dataset, test_scales=None, logger=None):
    if logger is None:
        logger = logging.getLogger(__name__)
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
        ...
        logger.debug(f'seg_shape {seg_preds.shape}, labels {labels.shape}, cam {cam.shape}')  # 调试信息
        ...
        if num % 1000 == 0:
            ...
            logger.info(f"Processed {num} samples - Current Scores:")
            logger.info(f"Seg Score: {seg_score}")
            logger.info(f"MSC Seg Score: {msc_seg_score}")
            logger.info(f"Cam Score: {cam_score}")
            _preds, _gts, _msc_preds, cams = [], [], [], []
...
def crf_proc(config, logger=None):
    if logger is None:
        logger = logging.getLogger(__name__)
    logger.info("Starting CRF post-processing...")
    ...
    logger.info(f"CRF Post-processing Results: {score}")
    return True
...
def main(cfg):
    ...
    logger.info("Starting CRF post-processing...")
    crf_proc(config=cfg, logger=logger)
